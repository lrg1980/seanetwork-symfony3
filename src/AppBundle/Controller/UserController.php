...
class UserController extends Controller
{
     ...
     public function loginAction(Request $request)
     {
          ...
          $authenticationUtils = $this->get('security.authentication_utils');   // Chequeado
          $error = $authenticationUtils->getLastAuthenticationError();          // Chequeado
          $lastUsername = $authenticationUtils->getLastUsername();              // Chequeado
     ...
     public function registerAction(Request $request)
     {
          ...
          if ($form->isSubmitted()) {
               if ($form->isValid()) {
                    ...
                    // Comprobación de si el nick está o no ya registrado. 
                    ...
                    if (count($user_isset) == 0) {
                         ...
                         if ($flush == null) {
                              ...
                              $status = "Bienvenido, te has registrado";
          ...
          if ($form->isSubmitted()) {
               if ($form->isValid()) {
                    ...
                    if (count($user_isset) == 0 || ($user->getEmail() == $user_isset[0]->getEmail() && $user->getNick() == $user_isset[0]->getNick())) {
                         ...
                         if ($flush == null) {
                              $status = "Has editado tus datos correctamente";
                         } else {
                              $status = "No has editado correctamente";
                         }
                    } else {
                         $status = "El usuario ya existe";
                    }
     ...
     public function profileAction(Request $request, $nickname = null){
          ...
          $user_id = $user->getId();
          $dql = "SELECT p FROM BackendBundle:Publication p 
                    WHERE p.user = $user_id ORDER BY p.id DESC";
                    $query = $em->createQuery($dql);
